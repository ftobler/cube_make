...
class MakefileGenerator:
    ...
    def _find_sources(self, extension):
        found_files = []
        source_file_dirs = set()
        for source_path in self.source_paths:
            full_path = os.path.join(self.project_path, source_path)
            for root, _, files in os.walk(full_path):
                for file in files:
                    if file.endswith(extension):
                        relative_dir = os.path.relpath(root, self.project_path)
                        if relative_dir == '.':
                            source_file_dirs.add('.')
                        else:
                            source_file_dirs.add(relative_dir)
                        found_files.append(file)
        self._vpath_dirs = sorted(list(source_file_dirs))
    ...
    def generate(self):
        ...
        vpath_dirs = " ".join(self._vpath_dirs)
